...
const DEFAULT_DATABASE_URL: &'static str = "mysql://root:mysql@localhost:5432/rust";
...
fn write_to_db(
...
) -> FutureResult<usize, hyper::Error> {
    use schema::messages;
    ...
    match inserted_id {
        Ok(inserted_id) => futures::future::ok(inserted_id),
        Err(error) => {
...
fn make_post_response(
    result: Result<usize, hyper::Error>,
...
fn query_db(time_range: TimeRange, db_connection: &MysqlConnection) -> Option<Vec<Message>> {
    use schema::messages;
...
fn connect_to_db() -> Option<MysqlConnection> {
